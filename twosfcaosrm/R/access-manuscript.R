#' Manuscript `access()`: ratio \eqn{R = 1/\sum P} then \eqn{A} per 10\,000
#'
#' This is the **thesis / pharmacy** formulation (not the generic 2SFCA step in
#' [access()]): for each supply site, \eqn{R = 1/\sum population} over tracts
#' whose catchment contains that site, then for each tract \eqn{A = \sum R
#' \times 10000}.
#'
...
access_step_ratio <- function(pnts,
  ...
  pnts %>%
    ...
    dplyr::summarize(A_sum = dplyr::first(.data$A_sum), .groups = "drop") %>%
